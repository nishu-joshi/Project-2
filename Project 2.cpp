...
public:
	cLane(int width)
	{
		...
		right = rand() % 2; //will decide whether the cars will enter from left or right.
	}
	void Move()
	{
		if (right) // if cars enter from right
		{
			if (rand() % 10 == 1) // if car will enter or not
				cars.push_front(true);
			else
				cars.push_front(false);
			cars.pop_back();
		}
		else  // else cars enter from left.
		{
			if (rand() % 10 == 1)
				cars.push_back(true);
			else
...
public:
	...
	void Input()
	{
		if (_kbhit())  // checks whether key is pressed or not.
		{
			char current = _getch();
			if (current == 'a')
				player->x--;
